...
<body>
  ...
  <table>
    ...
    ${row('Volume', `${p.cft ?? 0} CBM`)}
    ${row('Rate/CBM', fp(p.rate_per_cft_paise ?? 0))}
    ${row('HSN', hsnCode)}
  </table>
  <hr class="rule">
        ...
        <div className="no-print" style={{ background: 'var(--bg2)', border: '1px solid var(--bd)', borderRadius: 8, padding: '16px 20px', marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
            <div style={{ background: 'var(--bg1)', border: '1px solid var(--bd)', borderRadius: 6, padding: '10px 12px' }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--t2)' }}>{matchData.ordered.blocks} blocks · {matchData.ordered.cft} CBM</div>
      ...
      <div className="receipt">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
          ...
          <div style={{ padding: '12px 14px', background: '#f7f5f2', borderRadius: 4, border: '1px solid rgba(26,22,18,.1)' }}>
            ...
            <POInfoRow label="Volume"     value={`${po.cft ?? 0} CBM`} />
            <POInfoRow label="Rate / CBM" value={formatINR(po.rate_per_cft_paise ?? 0)} />
          ...
          {showGRNForm && (
            <form onSubmit={handleCreateGRN} style={{ padding: 16, borderTop: '1px solid var(--bd)', backgroundColor: 'var(--bg1)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
                ...
                <div><label className="fl">CBM Received *</label><input type="number" step="0.01" value={grnForm.cft_received || ''} onChange={e => setGrnForm({ ...grnForm, cft_received: parseFloat(e.target.value) || 0 })} required className="fi" min="0" /></div>
                <div>
            ...
            <div style={{ borderTop: '1px solid var(--bd)' }}>
              {grns.map((g: any) => (
                <div key={g.id} style={{ display: 'flex', gap: 16, padding: '10px 16px', borderBottom: '1px solid var(--bd)', fontSize: 12, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  <span>{g.blocks_received} blocks · {g.cft_received} CBM</span>
                  {g.net_weight_kg > 0 && (
