...
function RaisePOModal({ insp, onClose }: { insp: any; onClose: () => void }) {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!rate) { toast('Enter rate per CBM', 'error'); return; }
    raise.mutate(
      ...
      <div style={{ background:'var(--bg0)', borderRadius:'16px 16px 0 0', padding:'24px 20px', width:'100%', maxWidth:540, paddingBottom:'env(safe-area-inset-bottom, 16px)' }}>
        ...
        <form onSubmit={submit}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            <div><label style={lbl}>Rate/CBM (₹)</label>
              ...
              {masterRate > 0 && (
                <div style={{ fontSize: 11, color: 'var(--t3)', marginTop: 3 }}>
                  Master price: {formatINR(masterRate)}/CBM
        ...
        {editing ? (
          <div style={{ display: 'grid', gap: 12 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div><label style={lbl}>Est. CBM</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px', fontSize: 13 }}>
            ...
            <div><span style={{ color: 'var(--t3)' }}>Est. CBM</span><br /><strong>{insp.est_cft}</strong></div>
